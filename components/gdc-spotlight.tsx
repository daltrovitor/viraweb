...
export default function GdcSpotlight() {
  ...
  return (
    <section id="gdc" className="py-24 bg-white border-b border-[#E2E8F0] relative overflow-hidden">
      ...
      {/* FULL-SCREEN MOBILE SIMULATOR WIZARD OVERLAY MODAL */}
      {showWizard && isMobile && (
        <div className="fixed inset-0 bg-white z-[99999] overflow-y-auto p-6 flex flex-col font-sans">
          
          {/* Modal Header */}
          <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-6 select-none shrink-0">
            <div>
              <h3 className="text-base font-black text-slate-900 leading-none">
                {language === 'en' ? 'GDC - Simulator' : 'GDC - Simulador'}
              </h3>
            ...
            <button
              ...
              className="text-slate-400 hover:text-slate-900 border border-slate-200 p-2 text-xs font-bold flex items-center justify-center rounded-lg cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          {/* Wizard Steps indicator bar */}
          <div className="flex items-center justify-between gap-1 mb-8 select-none shrink-0">
            {[
              { num: 1, label: language === 'en' ? 'Client' : language === 'es' ? 'Cliente' : 'Cliente', icon: <User className="w-3 h-3" /> },
              { num: 2, label: language === 'en' ? 'Products' : language === 'es' ? 'Productos' : 'Produtos', icon: <ShoppingBag className="w-3 h-3" /> },
              { num: 3, label: language === 'en' ? 'Payment' : language === 'es' ? 'Pago' : 'Pagamento', icon: <CreditCard className="w-3 h-3" /> },
              ...
              <div key={step.num} className="flex-1 flex items-center gap-1.5">
                ...
                {step.num < 4 && <div className="flex-1 h-[1px] bg-slate-100" />}
              </div>
            ))}
          </div>
          ...
          <div className="flex-1 min-h-[280px]">
              ...
              <div className="flex flex-col">
                ...
                <div className="relative mb-4">
                  ...
                  <input
                    ...
                    value={selectedClient}
                    onChange={(e) => setSelectedClient(e.target.value)}
                    className="w-full bg-white border border-slate-200 pl-9 pr-4 py-2.5 text-xs focus:outline-none focus:border-blue-500 font-medium"
                  />
                </div>
                ...
                <div className="space-y-1.5 font-bold text-xs text-slate-600">
                  {availableClients
                    ...
                    .map(c => (
                      ...
                      >
                        <span>{c}</span>
                        {selectedClient === c && <Check className="w-4 h-4 text-blue-600" />}
                      </div>
                    ))}
                </div>
              </div>
                  ...
                  {availableServices.map(service => {
                    ...
                    return (
                      <div
                        key={service.name}
                        onClick={() => handleAddService(service)}
                        className={`p-3 border cursor-pointer flex justify-between items-center text-xs font-bold transition-all ${
                          isAdded 
                            ? 'border-blue-500 bg-blue-50/20 text-blue-700 font-bold' 
                            : 'border-slate-100 hover:bg-slate-50 text-slate-700'
                        }`}
                ...
                <div className="grid grid-cols-1 gap-2.5 text-xs font-bold text-slate-700">
                  {[
                    ...
                    return (
                      ...
                      >
                        <span>{translatePaymentMethod(method)}</span>
                        {paymentMethod === method && <Check className="w-4 h-4 text-blue-600" />}
                      </div>
                    );
                  })}
                </div>
            ...
            {wizardStep === 4 && (
              /* STEP 4: SUMMARY/SUBMIT */
              <div className="bg-slate-50 border border-slate-100 p-5 text-xs flex flex-col">
                <h4 className="font-bold text-slate-800 uppercase tracking-wider border-b border-slate-200 pb-2 mb-4">
                  {language === 'en' ? 'Proposal Summary' : language === 'es' ? 'Resumen de la Propuesta' : 'Resumo da Proposta'}
                </h4>
                
                <div className="space-y-3.5 font-bold text-slate-700">
                  <div className="flex justify-between">
                    <span className="text-slate-400">
                      {language === 'en' ? 'Client' : language === 'es' ? 'Cliente' : 'Cliente'}
                    </span>
                  ...
                  <div className="border-t border-slate-200/60 pt-3">
                    ...
                    {selectedServices.map(s => (
                      <div key={s.name} className="flex justify-between font-bold text-slate-800 py-1">
                        <span>{translateItem(s.name)}</span>
                        <span className="font-mono">R$ {s.price.toLocaleString('pt-BR')}</span>
                      </div>
                    ))}
          ...
          {/* Wizard Navigation Actions */}
          <div className="flex justify-between items-center mt-6 pt-4 border-t border-slate-100 select-none shrink-0 pb-6">
            <button
              onClick={() => setWizardStep(prev => Math.max(1, prev - 1))}
              disabled={wizardStep === 1}
              className="px-5 py-3 border border-slate-200 text-xs font-bold text-slate-600 hover:bg-slate-50 cursor-pointer disabled:opacity-40 rounded-lg"
            >
              {language === 'en' ? 'Back' : language === 'es' ? 'Volver' : 'Voltar'}
            </button>
            ...
            {wizardStep < 4 ? (
              <button
                onClick={() => setWizardStep(prev => prev + 1)}
                disabled={wizardStep === 1 ? !selectedClient : wizardStep === 2 ? selectedServices.length === 0 : false}
                className="bg-[#2563EB] hover:bg-blue-700 text-white font-bold text-xs px-8 py-3 flex items-center gap-1.5 cursor-pointer disabled:opacity-40 rounded-lg"
              >
                {language === 'en' ? 'Next' : language === 'es' ? 'Avanzar' : 'Avançar'}
                <ArrowRight className="w-4 h-4" />
              </button>
              ...
              <button
                onClick={handleCreateBudget}
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-8 py-3 flex items-center gap-1.5 cursor-pointer rounded-lg"
              >
                {language === 'en' ? 'Generate Budget' : language === 'es' ? 'Generar Presupuesto' : 'Gerar Orçamento'}
                <Check className="w-4 h-4" />
              </button>
