...
export default function SplashCursor({
  ...
  TRANSPARENT = true,
  RAINBOW_MODE = true,
  COLOR = '#2563EB' // Tech Accent primary (Royal Blue)
}: SplashCursorProps) {
  ...
  useEffect(() => {
    ...
    // Set initial size immediately to prevent division by zero (NaN)
    ...
    function step(dt: number) {
      ...
      // Gradient Subtract
      gradSubProgram.bind();
      gl.uniform2f(gradSubProgram.uniforms.texelSize, velocity.read.texelSizeX, velocity.read.texelSizeY);
    ...
    const virawebColors = ['#2563EB', '#06B6D4', '#D97706'];

    function generateColor() {
      if (!config.RAINBOW_MODE) {
        return hexToRGB(config.COLOR);
      }
      const randomHex = virawebColors[Math.floor(Math.random() * virawebColors.length)];
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      if (cursorRingRef.current) {
        cursorRingRef.current.style.transform = `translate3d(${e.clientX - 13}px, ${e.clientY - 13}px, 0)`;
      }
  ...
  return (
    <>
      ...
      {/* hardware-accelerated fallback cursor ring & dot */}
      <div
        ref={cursorDotRef}
        style={{
          position: 'fixed',
