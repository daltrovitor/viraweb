...
@keyframes footer-heartbeat {
  0%, 100% { transform: scale(1); filter: drop-shadow(0 0 5px rgba(220, 38, 38, 0.4)); }
  15%, 45% { transform: scale(1.2); filter: drop-shadow(0 0 10px rgba(220, 38, 38, 0.7)); }
...
.footer-giant-bg-text {
  ...
  background: linear-gradient(180deg, rgba(15, 23, 42, 0.08) 0%, transparent 60%);
...
.footer-text-glow {
  ...
  background-clip: text;
  filter: drop-shadow(0px 0px 20px rgba(37, 99, 235, 0.15));
}
`;
...
export function CinematicFooter() {
  ...
  return (
    <>
      ...
      <div
        ref={wrapperRef}
        className="relative h-[90vh] w-full border-t border-slate-100 bg-[#F8FAFC]"
        style={{ clipPath: "polygon(0% 0, 100% 0%, 100% 100%, 0 100%)" }}
      >
        <footer className="fixed bottom-0 left-0 flex h-[90vh] w-full flex-col justify-between overflow-hidden bg-[#F8FAFC] text-slate-800 cinematic-footer-wrapper">
          ...
          <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 mt-20 w-full max-w-5xl mx-auto text-center">
            ...
            <div ref={linksRef} className="flex flex-col items-center gap-6 w-full">
              ...
              <div className="flex flex-wrap justify-center gap-3 md:gap-6 w-full mt-2 font-semibold">
                ...
                <a href="#leadscrap" className="footer-glass-pill px-5 py-2.5 rounded-full text-slate-500 font-bold text-xs md:text-sm hover:text-slate-950 hover:border-slate-300 transition-all">
                  LeadScrap
                </a>
              </div>
            </div>
          ...
          <div className="relative z-20 w-full pb-8 px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <MagneticButton
              onClick={scrollToTop}
              className="w-10 h-10 rounded-full footer-glass-pill flex items-center justify-center text-slate-400 hover:text-slate-950 hover:border-slate-300 group order-3 border-slate-200/50"
            >
